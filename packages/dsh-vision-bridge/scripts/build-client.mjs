/**
 * Wrap the tsc-compiled client bundle for the DSH module loader.
 * Adapted from @anionex/dsh-vision-toolkit's scripts/build-client.mjs (MIT).
 */
...
push('window.__ModuleLoader__.load({ id: "dsh-vision-bridge", factory: (require) => {')
...
for (const filename of compiledFiles) {
  ...
  const source = (await readFile(compiledPath, 'utf8'))
    ...
    // CommonJS imports through the private module table.
